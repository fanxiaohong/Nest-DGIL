...
parser.add_argument('--cs_ratio', type=int, default=40, help='from {10, 25, 30, 40, 50}')
...
parser.add_argument('--overlap_size', type=str, default=8, help='overlap pixel: 8')
...
def get_val_result(model,mode,save_image_str, test_name, data_dir_org, overlap_size):
    ...
    with torch.no_grad():
        ...
        # for img_no in range(1):
            ...
            start = time()
            [Iorg, row, col, Ipad, row_new, col_new,Img_rec_yuv] = imread_CS_py(imgName)
            patches = img2patches(Ipad, (33, 33), (step, step))
            ...
            for idx, list_data in enumerate(BatchSampler(inputs, batch_size=args.eval_batch_size, drop_last=False)):
                ...
                [output[list_tmp, :, :, :], Phi] = model(batch_x, n_input)
